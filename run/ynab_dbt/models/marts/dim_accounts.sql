...
create or replace transient table PERSONAL_FINANCE.analytics_prod.dim_accounts
    ...
    as (with  __dbt__cte__stg_ynab_accounts as (
with raw_data as (
    select payload
...
lateral flatten(input => payload:data:accounts) as account_node
    ...
    select * from __dbt__cte__stg_ynab_accounts
)
